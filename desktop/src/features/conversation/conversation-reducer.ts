import { readRecord, readString, type BridgeEvent } from "../../lib/bridge/protocol";
...
export function conversationReducer(
  ...
  switch (event.event) {
    ...
    case "agent.reasoning.delta":
      return updateTurn(state, event.turnId, (turn) => ({
        ...turn,
        reasoningText: turn.reasoningText + readString(event.payload, "content"),
        status: "streaming"
      }));
...
function createTurn(event: BridgeEvent): ConversationTurn {
  return {
    id: event.turnId ?? 0,
...
function previewText(text: string): string {
  const normalized = cleanToolUseError(text).replace(/\s+/g, " ").trim();
...
function cleanToolUseError(text: string): string {
